...
const char * const BITCOIN_CONF_FILENAME = "shitcoin.conf";
const char * const BITCOIN_PID_FILENAME = "shitcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "shitcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Shitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Shitcoin
    // Mac: ~/Library/Application Support/Shitcoin
    // Unix: ~/.shitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Shitcoin";
#else
    // Unix
    return pathRet / ".shitcoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Shitcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Shitcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Shitcoin Core developers";
    }
