...
#if defined(NDEBUG)
# error "Shitcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Shitcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SHIT additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 0){ 
        ...
        // THIS IS SO WE CAN SCAM MORE PPL XDDDDDDDDD
    //    nSubsidy = 1000 * COIN
    //}
    // WANTED TO IMPLEMENT RANDOM BLOCK REWARDS, TOO LAZY XD

        // Subsidy is cut in half every 100,000 blocks which will occur approximately every idk seconds.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
void ThreadScriptCheck() {
    RenameThread("shitcoin-scriptch");
