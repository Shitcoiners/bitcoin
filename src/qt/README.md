This directory contains the ShitcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Shitcoin UI.
...
ClientModel is used by the main application `shitcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/shitcoin/shitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a shitcoin:... link from a browser).
...
* UI elements like ShitcoinAmountField, which inherit from QWidget.
* `shitcoinstrings.cpp`: automatically generated
* `shitcoinunits.(h/cpp)`: SHIT / mSHIT / etc handling
* `callback.h`
...
* don't change `local/shitcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "shitcoin-qt" under "Run", which is where you can also add command line arguments)
